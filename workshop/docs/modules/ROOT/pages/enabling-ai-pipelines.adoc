...
= (Optional) Enable AI pipelines

[role="_abstract"]
You must prepare your {deliverable} environment to use ai pipelines.

NOTE: If you do not intend to complete the pipelines section of this {deliverable}, you can skip to the next section, xref:setting-up-kueue-resources.adoc[Set up Kueue resource].

Later in this {deliverable}, you implement an example pipeline by using the JupyterLab Elyra extension. Elyra enables you to create a visual, end-to-end pipeline workflow that runs in {productname-short}.

.Prerequisites

* You have installed local object storage buckets and created connections, as described in xref:storing-data-with-connections.adoc[Configure storage credentials].
...
+
image::projects/ds-project-pipeline-server-view.png[Manage pipeline server configuration, 250]
+
An information box opens and displays the object storage connection information for the pipeline server.

.Next step

If you want to complete the _Distribute training with the Training Operator_ section of this {deliverable}, go to xref:setting-up-kueue-resources.adoc[Set up Kueue resource].

//NOTE: The _Distribute training with the Training Operator_ section of this {deliverable} is not supported if you are using the {org-name} Developer Sandbox.

Otherwise, skip to xref:creating-a-workbench.adoc[Create a workbench].

...
== Use machine learning features for training and inference
...
Feature Store in {productname-long} is based on the Feast open-source project. The Feature Store central feature registry provides a single place to define, discover, and manage features. It offers the following capabilities:
...
* link:https://feast.dev/[Feast^]
* xref:setting-up-a-local-feature-store.adoc[Set up a local feature store]

...
* You created a `My Storage` connection as described in xref:storing-data-with-connections.adoc[Configure storage credentials].

* If you intend to complete the pipelines section of this {deliverable}, you configured a pipeline server as described in xref:enabling-ai-pipelines.adoc[Enable AI pipelines].

* If you intend to complete the _Distribute training with the Training Operator_ section of this {deliverable}, you completed the procedure described in xref:setting-up-kueue-resources.adoc[Set up Kueue resource].
//Note that the _Distribute training with the Training Operator_ section of this {deliverable} is not supported in the {org-name} Developer Sandbox.

.Procedure

. Navigate to the project detail page for the project that you created in xref:setting-up-your-project.adoc[Create your project].
...
.. Select the image that includes *Tensorflow*:
+
*Jupyter | Tensorflow | CUDA | Python 3.12*

.. For the image version, select *2025.2*.
+
image::workbenches/create-workbench-form-image.png[Workbench image, 600]

. Under the *Deployment size* section, for *Hardware profile*, select *`default-profile`*.

. Keep the default environment variables and storage options.
+
...
xref:importing-files-into-jupyter.adoc[Import tutorial files]
